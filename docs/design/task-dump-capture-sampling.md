...
The implementation should add a workspace binary at
`examples/mixed-profile-test-app`. This is a normal runnable application, not
only a test fixture. It records a trace that can be opened in the viewer and
can also be generated by integration tests.

Its canonical task repeats a synchronized 600 ms cycle:

```text
expected_16_67_percent_on_cpu                    100 ms
expected_33_33_percent_waiting_for_blocking_lock 200 ms
expected_50_00_percent_waiting_for_timer         300 ms
```

The function names intentionally state their expected share of wall time so a
flamegraph is understandable without consulting the source. Mark the leaf
functions `#[inline(never)]` and use `black_box` where needed to preserve
recognizable symbols in release builds.

The CPU phase performs measured busy work in the instrumented task. For the
blocking-lock phase, a named OS thread holds a `std::sync::Mutex` while a
...
- a second instrumented task with a different by-construction percentage mix,
  proving that task-scoped profiles do not bleed into each other;
- an uninstrumented background CPU thread and a named off-CPU thread, proving
  that process activity does not enter a selected task's mixed graph;
- enough concurrent tasks and repeated cycles to drive every worker above the
  requested capture rate and make profiler and task-dump estimates stable;
- CLI options for trace directory, cycle count, worker count, and task-dump
  capture rate; and
- a machine-readable manifest containing task labels, configured durations,
  expected percentages, and the workload's monotonic start and end.

The first sampler epoch is warm-up. The integration harness derives its
steady-state measurement window from the per-worker `sampling_active_ns`
segment metadata and the manifest's workload end. It does not infer or
hard-code an attach-time offset.
...
Run `mixed-profile-test-app`, load its emitted trace through the production
decoder and mixed-flamegraph builder, and assert:
...
not exact sample-count assertions. Preserve a generated trace as a viewer
fixture only if the repository chooses to make the app's mixed flamegraph part
of the stable UI contract.
