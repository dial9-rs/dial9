...
| J6. Scope matching | Time window -> minute-level listing prefixes (<= 2h span, 2-min pad) or hour-level (> 2h, 1-hour pad, 72h cap) from the key layout `{prefix}/{YYYY-MM-DD}/{HHMM}/{service}/{host}/...`; then per-file interval-overlap on the FILENAME epoch (`{ts}-{i}.bin.gz`, padded by the segment duration; keys with no parseable epoch are kept). `service` = exact match on the key's service component; `host` = set membership on the key's host component (repeatable param, OR semantics - verified live: `host=local` matches the demo key, `host=local&host=nonexistent` still matches, `host=host-0` 404s). | `start_ns`/`end_ns`/`service`/`host` params. | `src/ingest/refine.rs:381-441` (`time_scoped_prefixes`); `src/ingest/aggregate.rs:163-178` (`parse_scope_fields`), `236-263` (`scope_matches`) |
