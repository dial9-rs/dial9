...
| B3. TZ toggle | Button labelled `TZ: UTC` / `TZ: Local` (default UTC), matching the browse and viewer pages. Governs BOTH directions of conversion: display (`nsToDatetime`: UTC slice of ISO string vs timezone-offset-shifted) and parse (`datetimeToNs`: appends `Z` when UTC, else local-time `Date` parse). Toggling re-renders the period inputs to the other convention (stored ns values unchanged) and re-labels them `From (UTC)` / `From (Local)`; does NOT re-render results or sync the URL. | Toolbar button "TZ: UTC". | `tokio_stats.html:57,73,75-85,414` |
