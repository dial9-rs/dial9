...
| C1a. Trace-embedded service/host identity | `[2026-07-13]` NEW (T45; closes #68). Surfaces the trace-EMBEDDED `service`/`host` from `ParsedTrace.segmentMetadata` (writer-emitted keys; producer S3 source or a DiskWriter `.segment_metadata()` caller) as chips in the file-info area. RECONCILED with the S3-key-derived `svc`/`host` URL params (W11, the OTHER source): embedded metadata WINS when both are present, and a disagreeing key-derived value is tooltipped; key-derived shows as a fallback when the trace has no embedded metadata. A field renders only when non-empty; the demo trace carries `service` but no `host`, so only the service chip appears there. | Toolbar row 1, after the filename. | `src/lib/trace/segment-metadata.ts` (`readSegmentIdentity`/`reconcileIdentity`, key contract); `src/pages/viewer/toolbar.ts` (`fileInfoTemplate` -> `identityTemplate`, `[data-file-identity]`); `src/types/trace.d.ts` (`SegmentIdentity`) |
