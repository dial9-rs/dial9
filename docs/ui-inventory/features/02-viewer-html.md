...
| D5. Uninstrumented popup | Fixed popup below the button: header "N uninstrumented task(s) at M site(s)"; hint text linking to `dial9::spawn` docs and the "Uninstrumented Polls" filter; sites grouped and sorted by count desc, each an auto-generated docs.rs link when the path matches a crate registry pattern, else plain text. Toggles off on repeat click; closes on `x`, click-outside, or Escape. | Click button; `x`/outside/Escape to close. | `viewer.html:957`, `5645-5717` |
...
| G13b. Task list scoped to the pin | `[2026-09-10]` NEW: with a spawn location pinned, the rail's Tasks tab lists only that family and shows a dismissible chip naming it, in the same control slot the Issues tab uses for its filter. `n`/`p` therefore step the family. The row cursor resolves from the SELECTED task rather than the stored `poi.taskIndex` ordinal, so a pin or sort change cannot leave it pointing at whatever task now sits at that position. Each row's spawn cell carries a hover `pin` button that sets the same state. | Rail Tasks tab. | `src/pages/viewer/tasks-model.ts` (`deriveTaskViewModel`); `src/pages/viewer/issues-rail.ts` (`pinChip`, `pinSpawnLoc`) |
...
| H10. Selection overlay | `#selection-overlay` div spans full main-area height during shift/alt drag or keyboard selection; blue for shift, teal for alt (`setSelOverlayColor`); `pointer-events:none`; cleared on mouseup/Escape. Shift selections are owned by the sidebar and persist until it closes. `[2026-09-11]` (#876) VERTICAL EXTENT: the box bounds the WORKER-LANES VIEWPORT (`selectionSpan` over `.d9-lanes-viewport`), not `0..scrollHeight` - the lanes are what a time selection is about, and every other track is a derived summary of the same window, so covering them tinted the ruler's labels, the span filter and the event chips for nothing. Bounding that element also makes the box track the one height the user drags by hand. Re-measured per render AND from a `ResizeObserver` on that same element, so it follows the lanes resize drag live; that drag withholds its height from the store until mouseup (so a shell re-render cannot fight it), leaving no store tick to ride. Landmarks come from `getBoundingClientRect`, never `offsetTop`, and the stack's bottom from the CONTAINER rather than its last child: #892 added a positioned overlay child to `.d9-tracks`, which made it the offset parent and silently slid the box back up over the ruler. `.d9-lanes-legend` is lifted over the box (z 6) ONLY while one is on screen: the overlay stamps `d9-has-selection-box` on the track column and CSS keys off it. The layering is otherwise a cycle - the box must sit over the lane canvas to be seen, the legend over the box to stay readable, and the legend under that same canvas the rest of the time, which is what stops this key hiding lane data. No static z-index satisfies all three, and shortening the box to clear the legend instead cost ~150px of a 449px viewport (it floats over the bottom third) and left the box short of the lowest workers. The POI fill is HATCHED rather than a flat wash (the span is wall time the worker was awake, only a fraction of it off-CPU) while its edges stay solid, since those are exact. `help.ts` carries a "Selection boxes" legend for the three colours plus the ruler bar - three boxes share one slot and differ only by colour, and always-on chrome would spend permanent space on a rare lookup. The extent is mode-independent, so the Shift region and Alt zoom boxes get it too - and since the box no longer starts at the column top, `--d9-lane-top` became BOX-relative (negative), so #898's measuring bar still lands in the ruler row. `[2026-09-10]` (#876) ATTRIBUTION: the box spans every lane, so its shape says nothing about WHICH worker, and its hard edges would read as the outage when the severity is a fraction of the boxed wall time. So `selection.poiRange` carries worker + severity + kind alongside the range, and three surfaces word it (`poiHighlightCaption` / `poiHighlightTitle` / `poiHighlightSummary`): an amber caption pinned inside the box's leading edge ("W0 · 1.41ms off-CPU", dropped below `CAPTION_MIN_WIDTH`) - it omits the boxed duration because #898's measuring bar states that one row above, and the pair is what stops the hard edges reading as a 17ms outage, the inspector status line ("W0 descheduled period selected"), and a card above the inspector tabs (window / awake / off-CPU with its share). The card sits ABOVE the tabs because the period is neither a task nor a poll, so every tab is the wrong home; before it, this was the one POI kind whose click left the inspector reading "No selection". `[2026-09-09]` (#876) A third, lowest-precedence tier: `selection.poiRange`, an amber edge-tinted box marking the current issues-rail jump. Written only for a POI the lanes draw no bar for (`off-cpu-active` - a descheduled worker period is a property of a stretch of wall time, not of a poll or a park), so without it that jump moves the viewport and marks nothing. Rebuilt from the anchored POI on load, so a shared link lands boxed; cleared by Escape, by the inspector's Clear, and by any rail control that drops the current row. | Visible during selection. | `viewer.html:728-735`, `5133-5142`, `5183-5209`; `src/pages/viewer/selection-overlay.ts` (`activeSelectionRegion`), `src/pages/viewer/poi.ts` (`poiJump().highlight`) |
...
| N3. "no wake data" badge | Red badge (links to `dial9::spawn` docs) for tasks spawned via raw `tokio::spawn` (uninstrumented). | Click badge. | `viewer.html:4404-4406`, `775-789` |
...
| N8b. Task scope (pinned spawn location) | `[2026-09-10]` the Task tab carries a `This task` / `All from spawn` switch that PINS a spawn location into `selection.scopedSpawnLoc`. One string drives three surfaces: the rail's Tasks tab filters to the family (G13b), the lanes tint its tasks (G8b), and the tab folds their samples. A location rather than a mode, so moving the selection cannot re-target the family silently; selecting a task from outside the pinned family leaves the pin standing and the tab describes that task, with an off-family note naming the pin. "All from spawn" is offered whenever the trace recorded a spawn location, including tasks spawned through raw `tokio::spawn` (tokio's task meta carries the location regardless of instrumentation). The profile is always rendered. Rides the URL as `task-scope=<location>`. | Task tab controls; rail row `pin` button. | `src/pages/viewer/task-flamegraph-model.ts`; `src/pages/viewer/task-scope-model.ts`; `src/pages/viewer/inspector.ts` (`taskScopeControls`, `familyStats`, `offFamilyNote`) |
