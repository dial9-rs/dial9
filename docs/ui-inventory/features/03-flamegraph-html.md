...
| F180. Browser URL sync (api) | Apply / any facet change rebuilds the full query (`api=1` + scope + facet state + picker times) and `history.pushState`s it - NOTE pushState, so Back walks the filter history (unlike exact-mode zoom's replaceState, F147). The migrated page also `replaceState`s `inspect`/`inspect_full`, carries the focus through scope changes, and retries restoration as streamed trees arrive. Canvas zoom remains NOT URL-synced. | Automatic on Apply/facet change and inspection changes. | legacy `flamegraph.html:202-219,336`; migrated `src/pages/flamegraph/api-mode.ts`, `view-state.ts` |
