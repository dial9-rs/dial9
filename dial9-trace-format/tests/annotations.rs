...
fn annotations_round_trip() {
    ...
    for frame in &frames {
        match frame {
            ...
            DecodedFrame::SchemaAnnotations {
                ...
                assert_eq!(*type_id, WireTypeId(dial9_trace_format::STATIC_WIRE_ID_LIMIT));
    ...
    let registry_entry = dec.registry().get(WireTypeId(dial9_trace_format::STATIC_WIRE_ID_LIMIT)).unwrap();
...
fn multiple_schemas_with_mixed_annotations() {
    ...
    let annotated_entry = dec.registry().get(WireTypeId(dial9_trace_format::STATIC_WIRE_ID_LIMIT)).unwrap();
    assert_eq!(annotated_entry.name(), "Annotated");
    assert_eq!(annotated_entry.annotations().len(), 1);

    let plain_entry = dec.registry().get(WireTypeId(dial9_trace_format::STATIC_WIRE_ID_LIMIT + 1)).unwrap();
...
fn annotations_unknown_type_id_skipped() {
    ...
    let real_entry = dec.registry().get(WireTypeId(dial9_trace_format::STATIC_WIRE_ID_LIMIT)).unwrap();
