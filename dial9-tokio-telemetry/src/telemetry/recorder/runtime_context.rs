...
mod shuttle_tests {
    ...
    /// caller threads. Every attach must be observed exactly once once all
    /// threads join, and the concurrent scan must never panic or deadlock.
    ///
    /// Deliberately does not touch `worker_id_base`/`worker_ids`: those are
