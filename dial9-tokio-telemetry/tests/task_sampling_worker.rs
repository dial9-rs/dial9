...
use dial9_tokio_telemetry::telemetry::{
    Dial9TokioHandle, MemoryBuffer, RecorderPipelineExt, TaskId, TaskSamplingConfig,
...
fn check_worker_capture(events: Vec<Dial9Event>, before: u64, after: u64) {
    ...
    let dumps = |id| {
        events
            ...
            .filter(|e| {
                matches!(e,
                    Dial9Event::TaskSampleEvent(d) if d.task_id == id
                )
            })
            .count()
    };
...
fn original_and_replacement_threads_can_sample_concurrently() {
    ...
    let rt = common::attach(
        ...
        1,
        TokioAttachOptions::builder()
            .task_sampling_config(TaskSamplingConfig::default())
            .build(),
    );
    let handle = Dial9TokioHandle::current();
...
fn long_poll_uses_pending_time_for_calibration() {
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, Dial9Event::TaskSampleEvent(_))),
...
fn nested_runtime_does_not_replace_the_enclosing_polls_sampler() {
    ...
    assert!(
        events.iter().any(|event| {
            matches!(event, Dial9Event::TaskSampleEvent(dump) if dump.task_id == task_id)
        }),
