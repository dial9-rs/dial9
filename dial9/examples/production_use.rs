...
fn configure_dial9(opts: &Dial9Opts) -> Recorder {
    if !opts.enabled {
        return dial9::recorder_disabled();
    }
    ...
    if let (Some(bucket), Some(service_name)) = (opts.s3_bucket.clone(), opts.service_name.clone())
    {
        ...
        let s3 = S3Config::builder()
            ...
            .build();
        return core.with_s3_uploader(s3).build();
    }
...
async fn main() {
    let tasks: Vec<_> = (0..100).map(|i| dial9::spawn(workload_task(i))).collect();
    for task in tasks {
        let _ = task.await;
    }
