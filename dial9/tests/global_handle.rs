...
/// Record one event from a thread no runtime owns, reporting whether
/// `Dial9Handle::current()` resolved there.
fn record_from_plain_thread(value: u64) -> bool {
    std::thread::spawn(move || {
        let live = Dial9Handle::current().is_enabled();
