//! Shared boot identifier and per-process namespace isolation.
//!
//! The `boot_id` (`{4-alpha}-{pid}`) stamps both the on-disk per-process
//! namespace (`{trace_dir}/{boot_id}/`) and the S3 object keys, so a process's
//! segments are attributable across both.
//!
//! Each process writes to `{trace_dir}/{boot_id}/` so background workers never
//! cross-process. Liveness is tracked via `flock(LOCK_EX)` on `{boot_id}/.lock`;
//! dead peers are GC'd at startup.
