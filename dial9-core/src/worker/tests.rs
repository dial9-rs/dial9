...
mod triggered_worker_tests {
    ...
    /// `epoch_cache` is disk-only (memory segments carry their epoch inline,
    /// with no file to re-read, so there is nothing to cache). Its entries
    /// are removed explicitly wherever this worker finishes with a segment,
    /// but a segment the writer evicts before any dump ever matches it
    /// never reaches any of those call sites -- only the `retain` at the
    /// top of the next matching pass catches that case. Drives
    /// `drain_matching` directly (bypassing `spawn_worker`'s background
    /// task) so the test can inspect `epoch_cache` between passes.
    #[tokio::test]
    async fn epoch_cache_prunes_writer_evicted_segments() {
        ...
        // Old segment, outside the narrow dump window registered below.
        let old_path = dir.path().join("trace.0.bin");
        std::fs::write(&old_path, segment_with_epoch(now - 3600)).unwrap();
        set_mtime(&old_path, now - 3600);

        let fs = Fs::new_disk(dir.path(), "trace");
        let mut worker = crate::worker::WorkerLoop::new(
            Arc::clone(&fs),
            Duration::from_millis(10),
        ...
        let req = crate::dump::DumpRequest {
            id: crate::dump::DumpId::new(),
            triggered_at: std::time::SystemTime::now(),
            lookback: crate::dump::Lookback::Window(Duration::from_secs(60)),
            lookforward: Duration::ZERO,
            metadata: Vec::new(),
            receipt_tx,
        };
        let mut dumps = vec![crate::worker::ActiveDump::register(req)];

        worker.drain_matching(&mut dumps).await;
        check!(
            worker.epoch_cache.len() == 1,
            "out-of-window disk segment should be cached, not re-read every pass"
        );
        check!(worker.epoch_cache.contains_key(&0));

        // Simulate the writer evicting the segment before any dump ever
        // matches it -- the one removal path with no explicit epoch_cache
        // cleanup of its own; only the next pass's `retain` catches it.
        std::fs::remove_file(&old_path).unwrap();

        worker.drain_matching(&mut dumps).await;
        check!(
            worker.epoch_cache.is_empty(),
            "evicted segment's cache entry must be pruned, not left dangling"
        );
    }
