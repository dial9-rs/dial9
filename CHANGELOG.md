...
`TokioAttachOptions`. 
`dial9::recorder_from_env()` builds a recorder and its runtime from the unchanged `DIAL9_*` vars. `dial9::recorder_or_disabled(writer)` and `dial9::recorder_disabled()` cover the boot-anyway and telemetry-off cases. `TracedRuntime`, the low-level `TracedRuntime::builder()`, and the pipeline / trace-path type-state markers are gone.
