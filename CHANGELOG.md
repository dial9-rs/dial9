...
- Opt-in process-global handle ([#699](https://github.com/dial9-rs/dial9/issues/699)).
  `Recorder::install_global()` publishes the recorder's handle process-wide and `Dial9Handle::global()` reads it from any thread, so a synchronous worker thread can record without a handle passed down to it.
