...
fn api_router(state: AppState) -> Router {
    ...
    router
        ...
        // Per-request metrics and dial9 API spans. Layered on the API router
        // (not the outer one) so it sees the populated `MatchedPath` and covers
        // API requests, not static-asset fetches.
