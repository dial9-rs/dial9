...
Pairs `SpanEnter`/`SpanExit` custom events into complete span objects. Requires the `tracing-layer` feature on `dial9` or `dial9-utils` and `Dial9TracingLayer` in the subscriber.
