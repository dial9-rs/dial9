...
#[tokio::test]
async fn services_discovers_current_services_and_previous_day_cutover() {
    let (s3, base, _dir) = setup_s3_test("traces-bucket", Some("traces-bucket".into()), None).await;
    let client = reqwest::Client::new();

    for key in [
        "version=1/date=2026-04-08/service=api/time=1925/instance=host-a/boot=boot/1000-0.bin.gz",
        "version=1/date=2026-04-08/service=previous-only/time=1925/instance=host-a/boot=boot/1000-0.bin.gz",
        "2026-04-09/1925/api/legacy-host/1000-0.bin.gz",
    ...
    let api_hint = metadata
        .iter()
        .find(|entry| entry["service"] == "api")
        .and_then(|entry| entry["layout_hint"].as_str())
        .unwrap();
    let decoded_hint: serde_json::Value = serde_json::from_str(api_hint).unwrap();
    check!(decoded_hint["first_v1_day"] == "2026-04-08");
    check!(body["truncated"] == false);
    ...
    check!(
        objects
            .iter()
            .all(|object| object["key"].as_str().unwrap().starts_with("version=1/"))
    );
}
...
#[tokio::test]
async fn wide_versioned_ranges_report_discovery_truncation() {
    let state =
        AppState::new(Arc::new(FakeBackend), Some("b".into()), None).with_time_partitioned_source();
    let base = start_server(state).await;
    let to = 2_000_i64 * 86_400; // 2,001 inclusive UTC days.

    for endpoint in ["services", "browse?service=api"] {
        let separator = if endpoint.contains('?') { '&' } else { '?' };
        let response = reqwest::Client::new()
            .get(format!(
                "{base}/api/{endpoint}{separator}bucket=b&from=0&to={to}"
            ))
            .send()
            .await
            .unwrap();
...
mod skills_unpack_tests {
    ...
    /// exactly what these tests must not depend on.
    fn unpacked_toolkit_dir(skills_root: &Path) -> PathBuf {
        std::fs::read_dir(skills_root)
            .unwrap()
            .map(|entry| entry.unwrap().path())
            .find(|dir| dir.join("scripts").join("trace_parser.js").is_file())
    ...
    fn analysis_tools_run_with_toolkit_dir_renamed() {
        ...
        let tools: [(PathBuf, Vec<&str>); 5] = [
            ...
            (
                scripts
                    .join("dial9-diagnose-long-poll")
                    .join("scripts")
                    .join("diagnose_long_poll.js"),
                vec![fixture],
            ),
            (
                scripts
            ...
            (toolkit.join("scripts").join("analyze.js"), vec![fixture]),
            (
        ...
        for (script, args) in &tools {
            ...
            assert!(
                output.status.success(),
                "{} failed with the toolkit renamed:\n{}",
                script.display(),
    ...
    /// locations it searched instead of failing on a source-tree path.
    #[test]
    fn analysis_tools_name_searched_locations_when_toolkit_is_missing() {
        let dir = unpack_skills();
        std::fs::remove_dir_all(unpacked_toolkit_dir(dir.path())).unwrap();
        ...
        let script = dir
            ...
            .join("scripts")
            .join("zoom.js");
        let output = run_node(&script, &["unused.bin", "100"]);
        assert!(!output.status.success());

        let stderr = String::from_utf8_lossy(&output.stderr);
        // Node reports paths through symlinks resolved, so compare canonical.
        let skills_root = dir.path().canonicalize().unwrap();
        assert!(stderr.contains("Searched:"), "stderr: {stderr}");
        assert!(
