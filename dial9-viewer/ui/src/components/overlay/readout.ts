// components/overlay/readout.ts - the at-cursor info readout (features/02 I6:
// the legacy top-right #info-panel) rehomed into the persistent inspector
// surface (04 S4: "at-moment stats in ONE place"). T24 computes the readout
// and writes it to the `transient.atCursor` store contract; T31's inspector
// renders it. Until T31 lands, `renderAtCursorStub` proves the wiring by
...
/** Nearest sample to `ns` by |t - ns| (legacy findNearest); binary search. */
...
/**
 * Windowed-data completeness at `ns` (T17 carried obligation - T17-audit
 * notes 6-7). Maps the covering segment's residency onto the readout's
 * coverage signal so the surface never presents a truncated window as whole:
 * an "oversized" covering segment -> "oversized"; a listed/fetching/evicted
 * one (partial data at this edge) -> "truncated"; a parsed one, or the
 * whole-trace (non-segmented) path where the map is empty, -> "complete".
 */
 ...
 * (`workerId`, null when off any lane). Mirrors the legacy `updateInfoPanel`:
 * nearest global-queue sample, MAX local-queue across workers, step
 * active-task count. `activeTaskCount` is null when the trace has no task
 * timeline (the legacy line was omitted then).
 */
...
// ── Inspector stub (proves the T31 wiring until T31 lands) ───────────────

const STUB_CLASS = "d9-atcursor-readout";

/** The inspector body the readout mirrors into (statically templated by T21). */
 ...
 * do not clobber it - the toast/legend technique. Null hides the readout.
 *
 * This is a STUB: T31's inspector will render `transient.atCursor` properly;
 * until then this proves the store contract is populated during hover.
 */
export function renderAtCursorStub(root: ParentNode, readout: AtCursorReadout | null): void {
  ...
  if (!host) {
    ...
    // Insert first so it reads as the "current moment" header (S4 intent).
