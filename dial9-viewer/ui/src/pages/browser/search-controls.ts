// Controls bar component (features/01 D): bucket/prefix inputs, prefix
// suggestion chips, quick-range buttons, From/To pickers, Search button.
...
export function mountSearchControls({ store, els, actions }: PageCtx): void {
  // D1/D10: keep the URL in sync as the user types; re-discover prefixes on
  // commit (change), detecting the bucket's region FIRST - discovery is
  // itself a signed S3 call (#607, legacy bucket listeners).
  ...
  // D2/D9: prefix edits gate the Search button and ride the URL.
  els.prefixInput.addEventListener("input", () => {
    actions.mirrorPrefix();
    actions.syncUrl();
  });

  // D6: quick-range buttons.
  ...
  // D8: a manual picker edit clears the quick-range highlight and flips the
  // URL from last=N to precise from/to.
  ...
  // D9: run the time-range (Browse) search.
  ...
  store.subscribe(["search"], (state) => {
    ...
    // D6 highlight: legacy matched button labels {1,3,24}; the data-hours
    // markup attribute carries the same values.
    for (const btn of els.quickBtns) {
    ...
    // D2 placeholder state cycling.
    els.prefixInput.placeholder = s.prefixPlaceholder;

    // D3 suggestion chips: rebuild only when the discovered set changes;
    // the active highlight toggles in place (chip clicks move it, manual
    // prefix edits deliberately do not - legacy behavior).
  ...
  // D9 `updateSearchReady`: disabled until a prefix is present when the
  // server declares one.
