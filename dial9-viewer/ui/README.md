...
| `field-chart` | **repeatable** `v1:[<id>,<event>,<field>,<kind>]` JSON tuple | Numeric custom-event chart definitions; `kind` is `gauge`, `counter`, or `updown-counter`. Dynamic ids may also appear in `track-order`/`collapsed`. |
