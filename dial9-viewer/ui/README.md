...
| `field-chart` | **repeatable** `v1:[<id>,<event>,<field>,<kind>]` JSON tuple | Numeric custom-event chart definitions; `kind` is `gauge`, `counter`, or `updown-counter`. Counter kinds render successive-observation deltas over their intervals. Dynamic ids may also appear in `track-order`/`collapsed`. |
