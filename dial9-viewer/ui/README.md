...
Evolution is additive-only: a documented param never changes meaning, never
gets removed, and never stops being honored by the page that owns it. New
capability means a NEW query param or hash key, and adding one requires
updating `URL_CONTRACT.md`, the ledger (`docs/tickets/ledger.md`), and the
...
Repository cross-references kept out of `URL_CONTRACT.md`:

- The browser page emits the exact-mode params via
  `traceTitleParams`/`objectTraceUrls`, and sets `segs` to
  `String(keys.length)`. Each `trace=` value behaves as described in "The
  `trace=` query parameter" above.
- Inventory rows: `worker-zoom` is features/03 F148/F150, `offworker-zoom`
  F149, zoom restore gating F151, api-mode history and its unsynced canvas
  zoom F168/F180, `prof` features/02 A14.
- The viewer clamps `start`/`end` to `recordMinTs`/`recordMaxTs` and
  canonicalizes its durable view state with `history.replaceState` after each
  settled store update; api mode `history.pushState`s every Apply/facet change.
- `url_state.js` owns the `index.html` params; its header is the field-level
  contract.

### Enforcement

- `src/lib/url/url-contract.test.ts` (vitest) pins `URL_CONTRACT.md` against
  the code: schema version, hash-key registry behavior (live keys round-trip,
  ...
  Renaming its section headings or table param names breaks that test by
  design.
- `live-checks/url-contract.mjs` (T12-style, live server) constructs the recipe
  URLs of `URL_CONTRACT.md` and asserts the pages honor them end-to-end; see
  "Live UI checks" below.
...
constructs the deep-link recipe URLs of `URL_CONTRACT.md` in plain Node (no
